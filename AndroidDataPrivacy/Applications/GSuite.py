...
def checkRequestHeaders(flow, headers, results):
	...
	elif ('User-Agent' in headers.keys() and headers['User-Agent'][:13] == 'Android-Gmail' and flow.source == ''):
		flow.source = 'GMail'

	elif ('User-Agent' in headers.keys() and headers['User-Agent'][:7] == 'Cakemix' and flow.source == ''):
